...
export default function Contacto() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'pais') {
      const seleccionado = paises.find(p => {
        const nombreEsp = p.name.nativeName?.spa?.common || p.name.common;
        return nombreEsp === value;
      });

      if (seleccionado) {
        setForm(prev => ({
          ...prev,
          nombreOficial: seleccionado.name.nativeName?.spa?.official || seleccionado.name.official,
        }));
      }
    }
  };
        ...
        <Col md={6} xs={12}>
          <Form onSubmit={handleSubmit}>
            ...
            <Form.Group>
              ...
              <Form.Control as="select" name="pais" value={form.pais} onChange={handleChange} required>
                <option value="">Seleccione un país</option>
                {paises.map((pais, idx) => {
                  const nombreEsp = pais.name.nativeName?.spa?.common || pais.name.common;
                  return (
                    <option key={idx} value={nombreEsp}>
                      {nombreEsp}
                    </option>
                  );
                })}
            ...
            <Form.Group>
              ...
              <Form.Control type="text" name="nombreOficial" value={form.nombreOficial} readOnly />
            </Form.Group>

            <Form.Group>
              <Form.Label>Mensaje *</Form.Label>
              <Form.Control as="textarea" rows={4} name="mensaje" value={form.mensaje} onChange={handleChange} required />
            </Form.Group>
                ...
                {contactos.length > 0 ? (
                  contactos.map((c, i) => (
                    <tr key={i}>
                      ...
                      <td>{c.mensaje}</td>
                      <td>
                        <Button variant="warning" size="sm" onClick={() => editar(i)}>Editar</Button>{' '}
                        <Button variant="danger" size="sm" onClick={() => eliminar(i)}>Eliminar</Button>
                      </td>
                    </tr>
                  ))
                ) : (
